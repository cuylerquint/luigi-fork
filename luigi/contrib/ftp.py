...
class RemoteFileSystem(luigi.target.FileSystem):
    ...
    def _sftp_put(self, local_path, path, atomic):
        ...
        if atomic:
            tmp_path = os.path.join(directory, 'luigi-tmp-{:09d}'.format(secrets.SystemRandom().randrange(0, 10_000_000_000)))
        else:
            tmp_path = normpath
    ...
    def _ftp_put(self, local_path, path, atomic):
        ...
        # random file name
        if atomic:
            tmp_path = folder + os.sep + 'luigi-tmp-%09d' % secrets.SystemRandom().randrange(0, 10_000_000_000)
        else:
            tmp_path = normpath
    ...
    def get(self, path, local_path):
        ...
        tmp_local_path = local_path + '-luigi-tmp-%09d' % secrets.SystemRandom().randrange(0, 10_000_000_000)
    ...
    def open(self, mode):
        ...
        if mode == 'w':
            ...
            temppath = '{}-luigi-tmp-{:09d}'.format(
                self.path.lstrip('/'), secrets.SystemRandom().randrange(0, 10_000_000_000)
            )
            try:
